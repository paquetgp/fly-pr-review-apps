...
if [ "$EVENT_TYPE" = "closed" ]; then
  ...
  flyctl apps destroy "$app-db" -y || true
  exit 0
fi
...
if [ -n "$INPUT_VM" ]; then
  flyctl deploy --remote-only --config "$config" --app "$app" --regions "$region" --image "$image" --strategy immediate --ha=false ${build_args} ${build_secrets} --vm-size "$INPUT_VMSIZE" --env PHX_HOST=$app.fly.dev
else
  flyctl deploy --remote-only --config "$config" --app "$app" --regions "$region" --image "$image" --strategy immediate --ha=false ${build_args} ${build_secrets} --vm-cpu-kind "$INPUT_CPUKIND" --vm-cpus $INPUT_CPU --vm-memory "$INPUT_MEMORY" --env PHX_HOST=$app.fly.dev
fi
